    ...
    'polygon-triangulation': [
      {
        ...
        detail: [
          'This is a theorem rather than a coincidence, and it makes an assertion available that ' +
            'costs nothing.',
          'It catches almost every ear-clipping bug. A clip that removed a reflex vertex, a diagonal ' +
            'that crossed an edge, an ear test that accepted a vertex with a point inside it.',
          'What the shape does change is the work, not the output size. A convex polygon finds an ' +
            'ear at the first vertex every time, while a comb has to walk past every reflex vertex ' +
            'before it finds one.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The second condition is the one people leave out, and leaving it out produces a ' +
            'triangulation whose triangles overlap while the vertex count still works out.',
          'Testing it naively is what makes ear clipping quadratic: every candidate is checked ' +
            'against every remaining vertex.',
          'The optimisation everyone reaches for — only test reflex vertices — is correct, because a ' +
            'convex vertex inside the ear would force a reflex one inside it too. It is the ' +
            'difference between usable and not on a few thousand vertices.'
        ],
      ...
      },
      {
        ...
        detail: [
          'That is why the comparison in this section is fair rather than rigged. The second row is ' +
            'literally the first one with some diagonals flipped, so nothing about the input has ' +
            'changed.',
          'It also explains what "optimal triangulation" can mean at all.',
          'Since the set of triangulations is connected by flips, a local rule that improves each ' +
            'quadrilateral can reach a global optimum. That is not true of most optimisation ' +
            'problems, and it is the reason the flip algorithm terminates at the right answer.'
        ],
      ...
      },
      {
        ...
        detail: [
          'Stating it as a determinant rather than as "compute the circumcentre and compare radii" ' +
            'matters for the same reason it mattered in the primitives section.',
          'The circumcentre of a nearly degenerate triangle is enormous and its radius is ' +
            'meaningless, whereas the determinant only ever needs its sign.',
          'The property is also checkable exhaustively, which is what makes Delaunay one of the few ' +
            'geometric outputs you can verify rather than eyeball.'
        ],
        example: 'The default mesh checks every one of its 108 triangles against all 60 vertices and ' +
          'finds 0 violations. That costs 7 531 predicate calls, 0 of which needed exact arithmetic.'
      },
      {
        ...
        detail: [
          'This is the entire incremental Delaunay algorithm.',
          'Insert a point, split the triangle containing it into three, then legalise the three ' +
            'outer edges. Each legalisation that flips exposes two more edges to test.',
          'The recursion terminates because every flip strictly increases the sorted vector of ' +
            'angles, so no configuration can repeat. Omitting the propagation is the classic bug: ' +
            'the mesh looks right near the inserted point and violates the property two triangles ' +
            'away.'
        ],
      ...
      },
      {
        ...
        detail: [
          'A skinny triangle is a bad interpolation basis. A value at a point inside it is dominated ' +
            'by two vertices that are close together, and barely influenced by the third.',
          'So terrain rendered from one shows visible creases, and a finite-element solution over ' +
            'one converges badly.',
          'Delaunay does not eliminate skinny triangles — two points very close together force one ' +
            'however the mesh is drawn. It produces as few as the point set permits, which is a much ' +
            'more useful guarantee than it first sounds.'
        ],
      ...
      },
      {
        ...
        detail: [
          'Confusing them produces triangles outside the polygon, which is the single most common ' +
            'triangulation bug in rendering code.',
          'Delaunay over a polygon\'s vertices fills the convex hull, including the notches. Ear ' +
            'clipping over a point set is not even defined.',
          'The bridge is constrained Delaunay: force the polygon edges in, then legalise everything ' +
            'else. A constrained edge may then violate the empty-circle property, because it is ' +
      ...
      },
      {
        ...
        detail: [
          'The count is a necessary condition, and this one is much closer to sufficient.',
          'Overlapping triangles push the total above the polygon\'s area, and a missed region ' +
            'pushes it below, while both keep the count intact.',
          'It is one shoelace sum per triangle, and it turns "the triangulation looked right in the ' +
            'picture" into a test that runs on every fixture in a second. Pair it with a check that ' +
            'every triangle is wound the same way as the ring, and very little is left to go wrong.'
        ],
