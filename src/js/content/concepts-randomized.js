    ...
    'monte-carlo-estimation': [
      {
        ...
        readAs: 'Sigma divided by the square root of N. That is the spread of one sample divided ' +
          'by the square root of how many you took.',
        detail: [
          'The rate is a property of averaging independent draws rather than of the problem. So it ' +
            'is identical for an integral, an area, a probability and a simulated queue.',
          'That makes the cost of accuracy predictable and brutal. Three more digits costs a million ' +
            'times the work.',
          'Because the exponent is fixed, every technique worth knowing attacks σ instead. The ones ' +
            'that appear to beat the rate, like stratification and quasi-Monte Carlo, do so by ' +
            'abandoning independence rather than by improving the average.'
        ],
        example: 'The demo measures a mean error of 1.083e-1 at 16 samples. At 4 096 it is ' +
          '7.898e-3 against a predicted 6.767e-3, and at 65 536 it is 1.590e-3 against a ' +
          'predicted 1.692e-3. That is a factor of 68 for 4 096 times the work, where the rate ' +
          'predicts 64.'
      },
      {
        ...
        readAs: 'The grid rule’s error falls like one over m squared per axis, but it needs m to ' +
          'the power d points. Monte Carlo’s error falls like one over the square root of N ' +
          'whatever d is.',
        detail: [
          'At a fixed budget the nodes per axis collapse as d grows. A budget of 4 096 points is ' +
            '4 096 nodes in one dimension and 2 nodes in twelve.',
          'So the grid’s accuracy falls off a cliff while the sampled one does not move.',
          'This is the curse of dimensionality stated as a crossover rather than as a slogan. It is ' +
            'why financial and physical simulations in tens of dimensions are sampled, while a ' +
            'one-dimensional integral never should be.'
        ],
        example: 'At the same 4 096 points the demo measures a grid error of 2.48e-9 at d = 1 ' +
          'and 7.98e-2 at d = 8. The sampled error goes only from 3.19e-3 to 1.11e-2 across that ' +
          'whole range, and the crossover is at d = 5.'
      },
      {
        ...
        readAs: 'The variance of the pair’s average is one plus rho, over two, times the ' +
          'variance of a single draw. Rho is the correlation between the two halves.',
        detail: [
          'The formula is the whole story. A monotone integrand makes ρ strongly negative and the ' +
            'variance collapses.',
          'An integrand symmetric about the middle of the domain makes ρ positive, and the pairing ' +
            'is worse than useless.',
          'There is no way to know which you have without either looking at the function or ' +
            'measuring. So a library that applies antithetic sampling unconditionally is making an ' +
            'unstated assumption about its caller’s integrand.'
        ],
      ...
      },
      {
        ...
        readAs: 'Subtracting c times the control’s deviation from its known mean gives a ' +
          'variance of one minus rho squared, times the original. That holds when c is rho times ' +
          'the ratio of the two standard deviations.',
        detail: [
          'Because the saving is 1 − ρ², a control correlated at 0.3 removes 9% of the variance and ' +
            'is not worth writing. One at 0.99 removes 98%.',
          'The optimal coefficient is estimated from the same samples, which introduces a bias that ' +
            'vanishes as N grows and is negligible past a few hundred draws.',
          'The correlation should still be reported rather than assumed, because a control variate ' +
            'that turns out to be uncorrelated is silent code that does nothing.'
        ],
        example: 'With x as the control the demo measures a correlation near 0.99 and a 60.16× ' +
          'reduction on ∫₀¹ eˣ dx. On the oscillating integrand the correlation is essentially ' +
          'zero and the reduction is 1.01×.'
      },
      {
        ...
        detail: [
          'What remains shrinks with the stratum width, so for a smooth integrand the error falls ' +
            'faster than 1/√N.',
          'The catch is that the draws are no longer identically distributed, so the sample-variance ' +
            'formula no longer estimates the estimator’s variance. It reports roughly the ' +
            'unstratified value while the actual error is orders of magnitude smaller.',
          'Code that stratifies the sampling and keeps the standard error bar is quoting an interval ' +
            'that is far too wide. That is the rare case of a statistical mistake erring towards ' +
            'caution.'
        ],
        example: 'The demo measures a stratified sample variance of 0.242093 against plain ' +
          'sampling’s 0.233670, which is no reduction at all. The error falls from 3.748e-3 to ' +
          '1.088e-6, a factor of 3 445. Over 200 seeds its interval covers the answer 100.0% of ' +
          'the time against a nominal 95%. Plain sampling measures 96.0%, antithetic 96.0% and ' +
          'the control variate 95.0%.'
      },
      {
        ...
        detail: [
          'Random points clump. Over N draws the largest empty gap is much bigger than 1/N, and ' +
            'every clump is wasted work.',
          'The van der Corput sequence writes the index in base two and reflects the digits about ' +
            'the point. That produces a set whose discrepancy is O(log N / N) rather than O(1/√N).',
          'The inequality then bounds the integration error directly, and it has no probability in ' +
            'it at all.',
          'That is also its limitation, because there is no error bar to report and the bound needs ' +
            'the integrand’s variation, which is usually unknown.'
        ],
      ...
      },
      {
        ...
        readAs: 'The expectation under p equals the expectation under q of f times the ratio of ' +
          'the two densities. That holds for any q that is non-zero wherever p is.',
        detail: [
          'A tail probability of 3e-5 needs about 32 000 plain draws to see a single hit. So a ' +
            'budget of 20 000 typically returns exactly zero with a standard error of exactly zero, ' +
            'which is a confident wrong answer carrying no warning at all.',
          'Shifting the sampling distribution into the tail and reweighting removes that entirely, ' +
            'and the estimator stays unbiased for any valid proposal.',
          'The variance, however, depends completely on the choice. A proposal with a lighter tail ' +
            'than the target has infinite variance while still producing plausible numbers.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The failure mode is counter-intuitive, because the obvious health signal points the wrong ' +
            'way.',
          'An over-shifted proposal puts almost every draw past the threshold, which looks ideal, ' +
            'while concentrating the weight on a few of them. The weight ESS collapses and the ' +
            'estimate degrades, and no amount of staring at the hit count reveals it.',
          'The same statistic reappears in 19.4 for correlated chains. In both places it is the ' +
            'number that turns "the sampler ran" into "the sampler produced information".'
        ],
        example: 'At a shift of 7 the demo puts 19 982 of 20 000 draws past the threshold, which ' +
          'is the best hit count in the table. The weight ESS is 75.4 and the relative error is ' +
