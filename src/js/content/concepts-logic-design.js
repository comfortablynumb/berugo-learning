    ...
    'hardware-description-and-verification': [
      {
        ...
        detail: [
          'That is the entire argument for hardware description languages, and it is the same '
            + 'argument as infrastructure-as-code.',
          'A schematic cannot be diffed, reviewed, generated by a loop or tested in isolation, '
            + 'and a value can.',
          'The demo\'s 4-bit adder is written as a loop instantiating one module four times.',
          'A 64-bit version is the same four lines with a different constant.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The simulator, the timing analyser and the place-and-route tool all read the flat '
            + 'netlist, so a bug in elaboration is a bug in everything after it.',
          'The hierarchy survives as a naming convention.',
          'That is exactly how a linker treats module boundaries in a program.',
          'It is also why a synthesis report can still tell you which source module its gates '
            + 'came from.'
        ],
      ...
      },
      {
        ...
        detail: [
          'This is the hardest adjustment for a programmer, and the model that works is a '
            + 'dataflow graph rather than a list of statements.',
          'Everything in a combinational block happens continuously and simultaneously.',
          'The only sequencing in the whole design comes from the clock edges.',
          'A description written as if it were a program produces either a mess of latches or a '
            + 'synthesis error, and both are the tool telling you the model is wrong.'
        ],
      ...
      },
      {
        ...
        detail: [
          'Past twenty or so inputs that stops being possible, and the honest thing is to report '
            + 'the fraction rather than to imply the rest.',
          'That is a discipline worth importing into software.',
          '"Sampled 400 of 131 072, seeded, reproducible" is a claim somebody can check and '
            + 'repeat, and "well tested" is not.',
          'The demo refuses exhaustive checks past its limit and says why, rather than quietly '
            + 'sampling.'
        ],
      ...
      },
      {
        ...
        detail: [
          'This is the single most transferable idea in the milestone.',
          'The demo\'s adder model adds two JavaScript numbers; the netlist ripples carries '
            + 'through gates.',
          'They share no structure, so agreement over the whole input space is evidence.',
          'A model that reused the circuit\'s decomposition would agree with its bugs too, which '
            + 'is exactly what happens when a test asserts what the code currently returns.'
        ],
      ...
      },
      {
        ...
        detail: [
          'They answer different questions and a design needs both.',
          'Simulation is the only one that shows the waveform: glitches, settling times, the '
            + 'order things arrive in.',
          'Equivalence is the only one that can say the function is right.',
          'Neither subsumes the other, which is why a hardware flow runs both and a software flow '
            + 'that only has examples is missing half the picture.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The demo makes the failure mode concrete.',
          'A hand-written corner-case list reaches 80% toggle coverage on the full adder, visits '
            + 'half the input space, and misses the injected bug entirely.',
          'Even a single vector toggles some wires, so toggle coverage saturates long before '
            + 'testing does.',
          'That is precisely the relationship line coverage has to correctness in software: a '
            + 'floor worth having and a goal worth distrusting.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The miter construction XORs the two designs\' outputs together and asks a solver '
            + 'whether the result can ever be 1.',
          'That is what lets a synthesis flow apply thousands of transformations to a '
            + 'million-gate design and still guarantee the result matches the source.',
          'It is the previous milestone\'s CDCL machinery doing industrial work.',
          'It is also the strongest argument in this course for learning how a SAT solver works.'
        ],
