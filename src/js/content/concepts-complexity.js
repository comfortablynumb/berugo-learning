    ...
    'time-complexity-classes': [
      {
        ...
        diagram: {
          definition: [
            'flowchart LR',
            '    A["a class is defined as:<br/>the languages some machine<br/>decides within a bound"] --> B["so the definition names<br/>a machine model"]',
            '    B --> C["P: deterministic, polynomial time"]',
            '    B --> D["NP: nondeterministic, the same bound"]',
            '    C --> E["change the model and the classes<br/>change with it — which is what<br/>quantum and circuits do"]',
            '    D --> E'
          ].join('\n'),
          caption: 'Complexity classes are statements about machines that happen to be used to talk about problems. Every surprising result in the area comes from varying the machine.'
        },
    ...
    'randomised-and-interactive-classes': [
      {
        ...
        diagram: {
          definition: [
            'flowchart LR',
            '    A["RP: never accepts<br/>a false claim"] --> D["a yes is certain,<br/>a no is a maybe"]',
            '    B["co-RP: never rejects<br/>a true one"] --> E["a no is certain,<br/>a yes is a maybe"]',
            '    C["BPP: both directions<br/>may be wrong"] --> F["neither answer is certain,<br/>and both are usually right"]'
          ].join('\n'),
          caption: 'The taxonomy is not about how much error is allowed but on which side it sits — which is exactly what decides whether repeating the algorithm helps.'
        },
    ...
    'circuits-and-non-uniform-computation': [
      {
        ...
        diagram: {
          definition: [
            'flowchart LR',
            '    A["a circuit has a fixed<br/>number of input wires"] --> B["it handles inputs of that<br/>length, and no other"]',
            '    B --> C["so you need a FAMILY:<br/>one circuit per n"]',
            '    C --> D["and nothing requires the family<br/>to be generated by a program"]',
            '    D --> E["which is what non-uniform means,<br/>and why circuits can decide<br/>undecidable languages"]'
          ].join('\n'),
          caption: 'A Turing machine is one finite object handling every length. A circuit family is infinitely many, and dropping the requirement that they be constructible is what makes the model strictly stranger.'
        },
