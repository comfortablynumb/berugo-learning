    ...
    'general-matching': [
      {
        ...
        detail: [
          'That is why Kuhn can mark each right vertex once per search and still be correct. Any ' +
            'alternating continuation from a vertex is determined by which side it is on, and the ' +
            'side is determined by the parity of the distance from the root.',
          'Remove bipartiteness and the parity stops being well defined, because a vertex on an odd ' +
            'cycle is reachable at both parities.',
          'Every consequence in this section follows from that one sentence. So does the fact that ' +
            'colouring, independent set and vertex cover are all easy on bipartite graphs and hard ' +
            'in general.'
        ],
      ...
      },
      {
        ...
        readAs: 'In the alternating search tree, an edge between two even-depth vertices completes a cycle of ' +
          ...
          'need this machinery.',
        detail: [
          'The even-even edge is the detection rule, and it is exact.',
          'In a tree of alternating paths, an edge between two even-level vertices closes a cycle of ' +
            'odd length. The two paths to the root have even lengths, and the closing edge adds one.',
          'Everything inside that cycle can be reached at either parity, depending on which way ' +
            'round you go. So the one-mark-per-vertex rule throws away exactly the alternative that ' +
            'the augmenting path needed.'
        ],
      ...
      },
      {
        ...
        readAs: 'Squash the whole odd cycle into a single vertex and the question is unchanged: a path exists ' +
          ...
          'all.',
        detail: [
          'The theorem is the licence to do something that looks like cheating. A whole cycle ' +
            'becomes one vertex, the search continues in a smaller graph, and the path found there ' +
            'corresponds to a real path in the original.',
          'The rearrangement inside the blossom is always possible, because an odd cycle with one ' +
            'exposed vertex can be matched in either direction around the ring.',
          'This was the first algorithm ever argued to be polynomial in the modern sense. The paper ' +
            'is where "good algorithm" as a synonym for polynomial time comes from.'
        ],
      ...
      },
      {
        ...
        detail: [
          'A bug that fires on 1.7% of inputs produces a service that is correct on every example ' +
            'anybody types by hand. It passes every unit test written from those examples, and ' +
            'quietly under-allocates in production.',
          'It cannot be found by property testing unless the property is "equals an independent ' +
            'maximum". Computing that independent maximum is the hard part, which is precisely why ' +
            'nobody writes the test.',
          'The only defence is an exhaustive oracle on small inputs, run deliberately.'
        ],
      ...
      },
      {
        ...
        detail: [
          'This is the most uncomfortable fact in the section, and the most useful.',
          'The graph is fixed and the edges are fixed. Only the order in which the adjacency lists ' +
            'happen to be built changes, and the answer changes with it.',
          'So a test suite that builds its fixtures in a convenient order — sorted, or in insertion ' +
            'order from a tidy literal — will never see the failure. A production system whose ' +
            'adjacency comes out of a hash map will see it intermittently.'
        ],
      ...
      },
      {
        ...
        detail: [
          'Maximum-cardinality and minimum-weight matching are so often conflated that the ' +
            'difference is worth one line. The first asks how many pairs; the second assumes ' +
            'everyone is paired and asks how much it costs.',
          'On a complete bipartite graph the first is trivial, and only the second is interesting.',
          'The Hungarian algorithm solves the second in O(n³), and it is worth knowing that it is ' +
            'min-cost flow specialised rather than a separate invention.'
        ],
      ...
      },
      {
        ...
        detail: [
          'Those two conditions together prove optimality without mentioning the algorithm that ' +
            'produced them.',
          'Any permutation costs at least the sum of all the potentials, because each of its cells ' +
            'is at least its own two potentials. The chosen permutation costs exactly that, so it ' +
            'is minimum.',
          'That is a proof a reviewer can check in a minute against a table of numbers. It is the ' +
            'reason this section checks the certificate rather than comparing against a second ' +
            'implementation.'
        ],
      ...
      },
      {
        ...
        detail: [
          'The intuition that "greedy is roughly right" comes from problems where it has a proof, ' +
            'like the minimum spanning tree and fractional knapsack. The assignment problem is not ' +
            'one of them.',
          'Taking the cheapest cell in row 1 can force an expensive cell in row 6, and nothing ' +
            'bounds how expensive.',
          'Because the answer is still a valid permutation with a plausible total, it looks like a ' +
            'reasonable approximation — right up until somebody computes the optimum.'
        ],
