...
(function (root, factory) {
  ...
  const ARITHMETIC = [
    ...
    { token: '⊗', reads: 'combined along a path',
      means: 'The semiring product: how two weights on consecutive edges combine into one. Addition in the tropical semiring, multiplication in the probability one.' },
    { token: '⊂', reads: 'is a strict subset of',
      means: 'Everything in the first collection is in the second, and the second holds something the first does not.' },
    { token: '⏎', reads: 'a newline in the source',
      means: 'Marks where a line break sits inside an example, so a rule about newlines - automatic semicolon insertion, the offside rule - can be written on one line.' },
    { token: '•', reads: 'the dot, or the parse position',
      means: 'In an LR item, the marker showing how far the parser has got through a production: `E → E • + T` means an E has been seen and `+ T` is still to come.' },
    { token: 'Γ', reads: 'gamma',
      means: 'Conventionally the OUTPUT alphabet of a transducer, where sigma is the input alphabet.' },
