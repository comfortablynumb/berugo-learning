...
(function (root) {
  ...
  function insight() {
    return '**The state encoding is the clearest example in this milestone of a choice that '
      ...
      + 'the cost profile is not, and in every case the right answer depends on what is scarce '
      ...
      + 'the deserialised-struct-with-an-impossible-field problem, and the reason a state type '
      + 'that makes bad states unrepresentable is worth more than a state type with a comment '
      + 'saying which combinations are valid. In hardware the fix has a name — safe state '
