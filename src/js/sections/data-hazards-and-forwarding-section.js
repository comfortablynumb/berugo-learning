...
(function (root) {
  ...
  function orientation() {
    return [
      '**A data hazard is an instruction reading a register that an instruction still in '
        ...
        + 'later. So a machine that only reads the register file gets a stale value. The '
      ...
      '**Read-after-write is the only kind that can happen in an in-order pipeline.** '
        ...
        + 'Write-after-write needs them to complete out of order. Neither is possible when '
  ...
  function insight() {
    return '**The double-hazard bug is the most instructive defect in this milestone, and not '
      ...
      + 'checks MEM/WB before EX/MEM picks the older of two producers. It is wrong only '
