...
(function (root) {
  ...
  function orientation() {
    return [
      '**A hardware description is data, not a drawing, and that is the whole reason the '
        ...
        + 'generatable and testable. A schematic is none of those things, which is why a '
  ...
  function insight() {
    return '**Hardware verification is the discipline software testing wishes it could '
      + 'afford.** It can afford it because the input space of a combinational block is '
      + 'finite and small. When a block has ten inputs, "we checked every case" is a sentence '
      + 'with a proof behind it. The whole culture that follows is what a team does when '
      + 'exhaustiveness is achievable. A reference model written separately from the '
      + 'implementation, an assertion for every property, coverage measured and argued about, '
      + 'sign-off criteria that are numbers. Most software cannot do that, but the '
      ...
      + 'and misses the injected bug. That is exactly how a codebase reaches ninety per cent '
