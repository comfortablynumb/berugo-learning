...
(function (root, factory) {
  ...
  const CONTROLS = [
    { id: 'st-load', kind: 'range', label: 'load factor', value: 80, min: 20, max: 87, step: 1, suffix: '%',
      note: 'Abseil grows at 7/8. A group probe stays cheap where a plain linear probe is already suffering.' },
    { id: 'st-capacity', kind: 'range', label: 'slots', value: 2048, min: 256, max: 8192, step: 256 },
    { id: 'st-deletes', kind: 'range', label: 'deleted fraction', value: 0, min: 0, max: 50, step: 5, suffix: '%' }
  ];

  const METRICS = [
    { id: 'st-groups', label: 'Groups per lookup', note: '1.0 means one cache line answered it' },
    { id: 'st-keycmp', label: 'Key comparisons', note: 'only tags that matched are checked' },
    { id: 'st-plain', label: 'Plain open addressing', note: 'slot probes for the same keys' },
    { id: 'st-false', label: 'False tag matches', note: '1 in 128 by construction' }
  ];
  ...
  function render() {
    return '<div class="grid-2">' +
      ...
      '<div class="card-body"><div id="st-group" class="mono" style="font-size:.8125rem"></div>' +
      ...
      '<div class="card-body"><div id="st-chart"></div><div id="st-legend"></div>' +
      ...
      '<div class="card-body"><div id="st-split" class="mono" style="font-size:.8125rem"></div>' +
