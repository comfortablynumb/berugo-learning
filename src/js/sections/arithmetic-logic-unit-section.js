...
(function (root) {
  ...
  function orientation() {
    return [
      ...
      '**Subtraction reuses the adder because of two\'s complement, and the operation code does '
        ...
        + 'The same two wires that select "subtract" also configure the datapath to perform '
        + 'it.',
      ...
      '**Flags for the logic operations are forced, not inherited.** The adder is still running '
        ...
        + 'than leaving whatever the adder computed. A flag that is stale is worse than '
  ...
  function insight() {
    return '**The flags are the place where a piece of hardware admits it does not know what '
      ...
      + 'overflow": carry for the unsigned reading, overflow for the signed one. It leaves '
      + 'the choice to the instruction that reads them. That is why C has unsigned wraparound '
      + 'defined and signed overflow undefined, and why `a < b` compiles to a different branch '
      + 'for each type. It is why mixing the two in one expression is the reliable way to '
      + 'produce a bug that only appears at the boundary. The second thing worth taking is the '
      + 'sharing argument, because it is a design pattern rather than a fact about ALUs. One '
      + 'expensive structure, several cheap paths beside it, and a multiplexer to choose. That '
      + 'is how a floating-point unit shares a multiplier array between multiply and fused '
      + 'multiply-add. It is how a cache shares one tag comparator across ways, and how a '
      + 'well-factored piece of software shares one hot path between several thin wrappers. '
      + 'The cost is the '
