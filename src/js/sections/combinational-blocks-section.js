...
(function (root) {
  ...
  function orientation() {
    return [
      ...
      '**A priority encoder is a chain, and that is why interrupt latency is what it is.** Each '
        ...
        + 'cannot, because the answer depends on order — which is the same distinction as a '
      ...
      '**Equality and magnitude cost differently, and the reason is structural.** Equality is a '
        ...
        + 'significant differing bit, which is a chain, and it is why an unsigned comparison and '
      ...
      '**A barrel shifter is why a variable shift is one cycle.** One multiplexer stage per bit '
        ...
        + 'any distance. The alternative — shifting one place at a time — is width stages, and '
        + 'a processor that did that would have a shift latency that depended on the operand, '
  ...
  function insight() {
    return '**The reason to know these six blocks by their cost curves is that they explain the '
      ...
      + 'and the model is written from the specification in arithmetic rather than derived from '
      ...
      + 'more than any number of examples, and where the space is too large to walk, saying so '
